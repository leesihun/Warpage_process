#!/usr/bin/env python3
"""
Warpage Analyzer용 명령행 인터페이스 함수들
...
def parse_command_line_args():
    """
    명령행 인수를 파싱하고 설정 딕셔너리 반환
    Parse command line arguments and return a configuration dictionary.
    
    Returns:
        dict: 분석 매개변수들을 포함하는 설정 딕셔너리 / Configuration dictionary with analysis parameters
    """
    config = DEFAULT_CONFIG.copy()
    
    # 도움말 요청시 표시 / Show help if requested
    ...
    for arg in sys.argv[1:]:
        if arg.startswith('--base='):
            ...
            config['include_advanced'] = True
        elif arg == '--no-advanced':
            config['include_advanced'] = False
...
def print_help():
    ...
    print("  --advanced           Include advanced statistical analysis")
    print("  --no-advanced        Exclude advanced statistical analysis")
...
def interactive_mode():
    ...
    stats_input = input(f"Include statistical analysis (y/n) [{'y' if config['include_stats'] else 'n'}]: ").strip().lower()
    if stats_input in ['y', 'yes']:
        config['include_stats'] = True
    elif stats_input in ['n', 'no']:
        config['include_stats'] = False
        
    # 3D plots
    threed_input = input(f"Include 3D surface plots (y/n) [{'y' if config['include_3d'] else 'n'}]: ").strip().lower()
    if threed_input in ['y', 'yes']:
        config['include_3d'] = True
    elif threed_input in ['n', 'no']:
        config['include_3d'] = False
        
    # Advanced statistics
    advanced_input = input(f"Include advanced statistical analysis (y/n) [{'y' if config['include_advanced'] else 'n'}]: ").strip().lower()
    if advanced_input in ['y', 'yes']:
        config['include_advanced'] = True
    elif advanced_input in ['n', 'no']:
        config['include_advanced'] = False
