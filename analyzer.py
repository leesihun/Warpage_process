...
from statistics import find_optimal_color_range, print_statistical_comparison, print_file_information
...
def analyze_warpage(config=None):
    ...
    Returns:
        tuple: (comparison_fig, folder_data) - The comparison figure and processed data
    """
    ...
    for folder in folders:
        ...
        if file_results:
            for i, (center_data, stats, filename) in enumerate(file_results, 1):
                ...
                print(f"   ✓ {file_id}: {filename} ({file_size})")
        else:
